...
6. **⚠️ 金句(highlight)**：每段剧情必须提炼一句最有冲击力的话作为highlight，要求：
   ...
   - 示例：「她笑着递过毒药，嘴里说的是关心」
7. **⚠️ 选项必须与剧情匹配**（本条必须严格遵守）：
   - 每个选项必须是上一段剧情的**自然延续和合理外延**
   - 选项必须包含剧情中出现过的**关键人物、地点、事件、物品**的引用
   - 例如：剧情提到"华妃冷笑"，选项就不能凭空出现"太后赏赐"
   - 例如：剧情提到"收到淑嫔的字条"，选项必须围绕"字条内容"或"淑嫔意图"展开
   - 选项中提到的每一个细节，必须在最近1-2段剧情中出现过
   - 如果上一段剧情没有出现皇帝，选项中就不能出现"侍寝"类的选项（除非剧情自然发展到此）
  ...
  "choices": [
    {
      ...
      "requires": [] // 该选项的前置条件列表（如需要特定NPC在场、需要特定flag等）
    },
    {
      "id": 2,
...
/**
 * 构建用户消息（包含最近历史 + 剧情锚点）
 * 关键改进：让 AI 生成选项时必须引用上一步剧情中的关键元素
 */
export function buildUserMessage(state: GameState, playerInput: string): string {
  ...
  const historyText = recentHistory.length > 0
    ? recentHistory.map(h =>
        h.role === 'narrator' ? `【剧情】${h.content.slice(0, 300)}...` : `【玩家选择】${h.content}`
      ).join('\n')
    : '';

  // 提取上一步剧情的关键元素（用于锚定选项）
  const lastNarration = recentHistory.length > 0
